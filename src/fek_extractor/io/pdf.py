...
def extract_text_whole(pdf_path: Path) -> str:
    try:
        ...
        log.warning("Text extraction not allowed for %s", pdf_path)
        return ""
...
def iter_lines_from_pdf(pdf_path: Path, laparams: LAParams | None = None) -> list[str]:
    laparams = laparams or LAParams(
        all_texts=True, line_margin=0.2, char_margin=2.0, word_margin=0.1, boxes_flow=None
    )
    lines: list[str] = []
    try:
        ...
        log.warning("Layout extraction not allowed for %s", pdf_path)
    except Exception as e:
        log.error("Failed to iterate lines for %s: %s", pdf_path, e)
    return lines
